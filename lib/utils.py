...
def accuracy(output, target, topk=(1,)):
    ...
    res = []
    for k in topk:
        correct_k = correct[:k].contiguous().view(-1).float().sum(0) # TODO 报错，是torch库版本问题
...
def least_square_sklearn(X, Y):
    ...
    reg.fit(X, Y)
    return reg.coef_ # intercept_等价于 bias/截距; coef_等价于 W/系数； coef_中的θ1、θ2...θn分别对应各特征值对标记的影响，θ>0, 正相关；θ<0，负相关
